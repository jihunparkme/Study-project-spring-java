...
class DatePartitionJobConfigTest {
    ...
    fun `success date partitioning job`() {
        val jobParameters = JobParametersBuilder()
            .addString("startDate", "2026-01-01")
            .addString("endDate", "2026-02-01")
            .addLong("timestamp", System.currentTimeMillis())
            .toJobParameters()

        // 2. 생성한 파라미터를 인자로 넣어 Job 실행
        val jobExecution = jobLauncherTestUtils.launchJob(jobParameters)

        // then: 1. 전체 Job 결과가 COMPLETED 인지 확인
        assertThat(jobExecution.status).isEqualTo(BatchStatus.COMPLETED)

        // then: 2. 생성된 StepExecution 확인 (Master 1개 + Slave 32일치 = 총 33개)
        val stepExecutions = jobExecution.stepExecutions
        println("총 실행된 Step 수: ${stepExecutions.size}")

        // 2026-01-01 ~ 2026-02-01은 총 32일이므로, 마스터를 포함해 33개의 스텝이 실행되어야 함
